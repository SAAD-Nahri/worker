...
from distribution_engine.wordpress_transport_cli import main
from source_engine.models import SourceItem


def _build_source_item() -> SourceItem:
    return SourceItem(
        item_id="item-wordpress-transport-cli-123456",
        source_id="src_test",
...
class WordPressTransportCliTests(unittest.TestCase):
    def test_wordpress_transport_cli_dry_run_does_not_mutate_records(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(payload["operation"], "create_draft")
            self.assertEqual(payload["request"]["payload"]["status"], "draft")

    def test_wordpress_transport_cli_execute_appends_success_and_refreshes_workflow(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            config_path = root / "wordpress_config.json"
            blog_path = root / "blog_publish_records.jsonl"
            ...
            self.assertEqual(latest_blog.wordpress_status, "draft_created")
            self.assertEqual(latest_blog.wordpress_post_id, "345")
    ...
    def test_wordpress_transport_cli_execute_records_failed_snapshot(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            config_path = root / "wordpress_config.json"
            blog_path = root / "blog_publish_records.jsonl"
            ...
            def _executor(*_args):
                raise WordPressRestTransportError("remote timeout")
            ...
            self.assertEqual(latest_blog.wordpress_status, "failed")
            self.assertEqual(latest_blog.last_error, "remote timeout")
            self.assertEqual(latest_blog_queue.queue_state, "blog_publish_failed")
