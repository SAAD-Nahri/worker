...
from distribution_engine.social_review_cli import main
...
def _build_source_item() -> SourceItem:
    return SourceItem(
        item_id="item-social-review-cli-123456",
        source_id="src_test",
...
class SocialReviewCliTests(unittest.TestCase):
    def test_social_review_cli_updates_package_and_refreshes_mapping(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            latest_package = load_latest_social_package_record(payload["social_package_id"], path=social_path)
            latest_review = read_social_package_review_records(path=review_path)[0]
            ...
            self.assertEqual(latest_package.approval_state, "approved")
            self.assertEqual(latest_review.updated_approval_state, "approved")
